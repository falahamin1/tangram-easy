"""
Easy Tangram — V-Representation PPO Training
=============================================
Trains a DeepSet actor-critic using the vertex representation (V-rep)
of each piece's PPL polytope as state.

State shape fed to the network: [batch, 4, 8]
  4 pieces × (4 vertices × 2 coords) = 8 features per piece
...
def train_v_rep(episodes: int = 1000):
    # ── 1. Hyperparameters ────────────────────────────────────────────────────
    HP = {
        "lr"               : 3e-4,   # V-rep typically stable with a higher LR
        "clip_eps"         : 0.2,
        "ppo_epochs"       : 5,
    ...
    # V-rep: 4 vertices × 2 coords = 8 features per piece
    ...
    for ep in range(episodes):
        ...
        if ep % 100 == 0:
            ...
            print(f"V-Rep rollout {ep:4d} | recent={avg:.2f} | best={best_moving_avg:.2f}")

        if ep % HP["render_interval"] == 0:
            env.inner.render(f"V-Rep_Easy_Ep_{ep}")

    # ── 4. Save ───────────────────────────────────────────────────────────────
    torch.save(model.state_dict(), "model_v_rep_easy_final.pth")
